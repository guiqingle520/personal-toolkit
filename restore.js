...
const original = `import { flushPromises, mount, type VueWrapper } from '@vue/test-utils'
...
function createStatsCategoryResponse() {
  return createSuccessResponse([
    ...
    { categoryName: '__UNCLASSIFIED__', count: 5 },
  ])
}
...
function createStatsTrendResponse() {
  return createSuccessResponse([
    ...
    { date: '2026-04-02', createdCount: 3, completedCount: 2 },
  ])
}
...
describe('TodoList reset behavior', () => {
  beforeEach(() => {
    ...
    fetchMock.mockImplementation(async (url, options) => {
      ...
      if (url.includes('/api/todos/1/sub-items') && (!options || options.method === 'GET')) return { ok: true, json: async () => createSubItemsResponse() }
...
fs.writeFileSync('src/components/TodoList.test.ts', original);
